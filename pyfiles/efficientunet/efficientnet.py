        ...
        class Encoder(nn.Module):
            ...
            def func_with_svd(self, L: torch.Tensor, rank: int = 50):
                ...
                reconstructed_aa  =reconstructed_aa.view(L.shape)
                 ...
                 # Obtain the remaining columns after the rank
                u_remaining = u[:, :, -10:]
                s_remaining = s[:, -10:]
                v_remaining = v[:, :, -10:]

                # Reconstruct the remaining feature tensor
                reconstructed_remaining = torch.matmul(torch.matmul(u_remaining, torch.diag_embed(s_remaining)),
                                                       v_remaining.transpose(1, 2))
                Encoder.supernoise = reconstructed_remaining.view(L.shape)
                Encoder.bases = u_truncated
